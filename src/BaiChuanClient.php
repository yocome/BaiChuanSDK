...
class BaiChuanClient
{
     ...
     * 存放日志，TOP缓存数据
     *
     * 注：原SDK为 常量 TOP_SDK_WORK_DIR；请更新时，记得替换！
     */
    public static $sdk_work_dir = "/tmp/";
     ...
     *
     * 注：原SDK为 常量 TOP_SDK_DEV_MODE；请更新时，记得替换！
     */
    public static $sdk_dev_mode = false;
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt)
	{
		...
		$logger->conf["log_file"] = rtrim(static::$sdk_work_dir, '\\/') . '/' . "logs/top_comm_err_" . $this->appkey . "_" . date("Y-m-d") . ".log";
	...
	public function execute($request, $session = null,$bestUrl = null)
	{
		...
		{
			$logger = new TopLogger;
			$logger->conf["log_file"] = rtrim(static::$sdk_work_dir, '\\/') . '/' . "logs/top_biz_err_" . $this->appkey . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
				date("Y-m-d H:i:s"),
				$resp
			));
